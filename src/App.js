...
class App extends Component {
  ...
  editTaskBtn(event) {
    // Code Flow for amending a task.
    let editPos = event.target.getAttribute("data-pos");
    let editValue = this.state.tasks[editPos]; // Find the task to be updated.

    let newButton = document.getElementById("submit_task_btn").cloneNode(true); // Updating the submit button for the Edit Functionality.
    let oldButton = document.getElementById("submit_task_btn");
    ...
    document
      ...
      .setAttribute("data-edit-pos", editPos);
  ...
  componentDidUpdate() {
    // Adding event listener for the Edit Buttons after render.
  ...
  onEditSubmit() {
    // Function to update state with amended task value.
    let currentTaskList = this.state.tasks;
    let editPos = document
      .getElementById("add_task_input")
      .getAttribute("data-edit-pos");
    ...
    document.getElementById("submit_task_btn").textContent = "Submit"
  }

  onAddSubmit() {
    // Function to add to state with newly added task.
    let newTaskToAdd = document.getElementById("add_task_input").value;
    let tempTasksList = this.state.tasks;
    tempTasksList.push(newTaskToAdd);
    this.setState({
      tasks: tempTasksList,
    });
    document.getElementById("task_form").reset();
  ...
  render() {
    return (
      // Create the Form with a TextBox and a Submit button.
      <div>
        <h1> To-Do-Task</h1>
        <form id="task_form" onSubmit={this.formSubmit}>
          <input
            ...
            minLength="1"
          ></input>
          <button type="Submit" id="submit_task_btn" onClick={this.onAddSubmit}>
            Submit
          </button>
